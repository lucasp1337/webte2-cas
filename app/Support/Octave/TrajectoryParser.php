 ...
 *
 * The Octave Blade template emits four marker-delimited blocks:
 *
 *   ---T---          time vector (column or row)
 *   ---END-T---
 *   ---X---          full state matrix (columns: x, x_dot, theta, theta_dot)
 *   ---END-X---
 *   ---THETA---      output matrix y (columns: cart_pos, angle) — used for
 *   ---END-THETA---  direct access to the two measured outputs
 *   ---FINAL_STATE--- last row of the state matrix as a single row
 *   ---END-FINAL_STATE---
 *
...
final readonly class TrajectoryParser
{
    ...
    public function parse(
        ...
        /** @var array<int, array{t: float, x: float, theta: float}> $samples */
        $samples = [];
        foreach ($t as $i => $time) {
            $stateRow = $xMatrix[$i] ?? [];
            $samples[] = [
